...
const corsOptions = {
  origin: [
    ...
    'http://127.0.0.1:3000',
    'http://127.0.0.1:5500',
   'https://adminsite-1-l247.onrender.com',
...
if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD) {
  ...
  return res.json({
    ...
    token: process.env.DEMO_JWT_TOKEN,
    });
  } catch (error) {
...
// Optional: Script to insert test user (run once to ensure adminn@example.com exists)
const insertTestUser = async () => {
  ...
  if (!existingUser) {
    const hashedPassword = await bcrypt.hash(plainPassword, 10); // secure hashing
...
app.post('/api/send-email', async (req, res) => {
  ...
  // Handle missing or invalid jobDetails
  const safeJobDetails = jobDetails || {
    title: 'Unknown',
    company: 'Unknown',
    location: 'Unknown',
    salary: 'N/A',
  ...
  try {
    const mailOptions = {
      ...
      text: `Thank you for applying for the position of ${safeJobDetails.title} at ${safeJobDetails.company}!\n\nJob Details:\n- Title: ${safeJobDetails.title}\n- Company: ${safeJobDetails.company}\n- Location: ${safeJobDetails.location}\n- Salary: ₹${safeJobDetails.salary} Lakhs P.A.\n- Type: ${safeJobDetails.type}\n- Work Type: ${safeJobDetails.workType}\n- Experience: ${safeJobDetails.experience}\n\nWe will get back to you soon.`,
    html: `  <!DOCTYPE html>
    <html>
      <head>
        <style>
      ...
      <body>
        <div class="container">
          ...
          <p>
            ...
            Thank you for applying for the position of <strong>${safeJobDetails.title}</strong> at
            <strong>${safeJobDetails.company}</strong>. We have received your application successfully.
          </p>

          <div class="job-details">
            <h3>📄 Job Details</h3>
            <ul>
              <li><strong>Title:</strong> ${safeJobDetails.title}</li>
              <li><strong>Company:</strong> ${safeJobDetails.company}</li>
              <li><strong>Location:</strong> ${safeJobDetails.location}</li>
              <li><strong>Salary:</strong> ₹${safeJobDetails.salary} Lakhs P.A.</li>
              <li><strong>Type:</strong> ${safeJobDetails.type}</li>
              <li><strong>Work Type:</strong> ${safeJobDetails.workType}</li>
              <li><strong>Experience:</strong> ${safeJobDetails.experience}</li>
            </ul>
          </div>

          <div class="footer">
            We will get back to you soon.<br />
            Thank you for your interest in joining ${safeJobDetails.company}!
          </div>
        </div>
      </body>
    </html>`
